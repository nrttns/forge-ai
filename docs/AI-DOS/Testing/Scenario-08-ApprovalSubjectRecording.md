...
Validate the producer-side procedure that records exactly one eligible reviewed completion subject into the Target-owned Pending Human Governance Approval Subject slot (`docs/Projects/ForgeAI/Planning/ProjectStatus.md` §6.1; `docs/Projects/ForgeAI/Mission/AGENTS.md` §5.5; `docs/AI-DOS/Workflows/ProjectStateUpdater.md` §4–§8), distinct from `docs/AI-DOS/Testing/Scenario-07-ApprovalSubjectResolution.md`, which validates bare/explicit approval intent *consuming* an already-recorded subject, not the act of recording one.
...
applying its recording-specific Mutation Sequence steps (`docs/AI-DOS/Workflows/ProjectStateUpdater.md` §5 rules 6–11).

The agent must not select `docs/AI-DOS/Workflows/TaskPlanner.md`, must not apply an acceptance or gate-closing transition as part of recording, and must not select `docs/AI-DOS/Commands/AgentAuditCommand.md`.
...
Expected: safe stop under `docs/AI-DOS/Workflows/ProjectStateUpdater.md` §5 rule 6. The candidate is never eligible to be recorded; §6.1 is not mutated.
...
Expected: idempotent no-op under `docs/AI-DOS/Workflows/ProjectStateUpdater.md` §7. The agent reports the existing record as still valid and performs no mutation. This is distinct from Case 10, where the recorded subject or its evidence differs.
...
Expected: recording does not route through or invoke `docs/AI-DOS/Workflows/TaskPlanner.md`, does not generate or select a candidate, and does not activate any capability. Current phase, roadmap, capability status, active task, and authorized-next-action fields remain unchanged.
...
Expected: the subsequent invocation is handled entirely by `docs/AI-DOS/Testing/Scenario-07-ApprovalSubjectResolution.md` Case 1 under the existing PR #267 contracts, unmodified by this scenario: the agent resolves the newly recorded subject, re-resolves its revision identity, and routes its gate-closing transition to `docs/AI-DOS/Workflows/ProjectStateUpdater.md`. Recording and approval remain two separate, sequential Human-Governed actions.
...
- Routing a recording task through `docs/AI-DOS/Workflows/TaskPlanner.md`.
...
- `docs/AI-DOS/Workflows/ProjectStateUpdater.md` is used for all fifteen cases; `TaskPlanner.md` is not invoked.
