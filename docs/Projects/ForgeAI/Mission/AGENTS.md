...
| Last Updated | 2026-07-22 (§5.6 rule 5 closure-path clarification added) |
...
5. Human Governance approval validly resolved under Section 5.5 additionally closes the active work unit when either: (a) the approval resolves through a recorded Section 6.1 subject sharing the active work unit's exact reviewed-subject identity; or (b) Human Governance explicitly names the exact subject identity under Section 5.5 rule 5, no Section 6.1 record conflicts with that named identity, and the named identity matches the active work unit's exact reviewed-subject identity. Under either path, applying the gate-closing transition for the resolved subject clears the §2/§3 record for that same reviewed-subject identity and resets ProjectStatus Section 12's exactly one authorized next action to await Human Governance continuation, as part of the same authorized mutation. A missing, conflicting, invalid, or drifted reviewed-subject identity under either path invalidates closure and requires safe stop rather than a gate-closing transition. Approval must not select, generate, activate, or execute a further work unit as part of, or immediately following, this closure.
...
| `1.10.0-draft` | 2026-07-22 | Clarified §5.6 rule 5 to close a determinism gap: it previously described Active Work Unit closure only through gate-closing a recorded Section 6.1 subject, while §5.5 rule 5 separately permits explicit Human Governance approval of an exactly named subject with no Section 6.1 record. Rule 5 now states both closure paths explicitly — (a) via a recorded Section 6.1 subject sharing the active work unit's exact reviewed-subject identity, or (b) via §5.5 rule 5 explicit named-identity approval with no conflicting Section 6.1 record and an identity match against the active work unit — and requires safe stop rather than closure on a missing, conflicting, invalid, or drifted identity under either path. This preserves the existing single-resolved-subject closure requirement and grants no independent Target authority to merge state, repository activity, or GitHub state. No other rule, section, Roadmap, DevelopmentPhases, root `AGENTS.md`, or `docs/AI-DOS/` artifact was touched; no work unit was selected, generated, activated, or executed by this clarification. |
